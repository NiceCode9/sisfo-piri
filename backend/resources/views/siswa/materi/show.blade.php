...
@section('content')
<div class="page-header d-flex justify-content-between gap-3"><div><h1 class="page-title">{{ $materi->judul }}</h1><p class="page-subtitle mb-0">{{ $materi->mataPelajaran->kode ?? '-' }} • {{ $materi->tipe }}</p></div><a href="{{ route('siswa.materi.index') }}" class="btn btn-nexus-outline btn-sm">Kembali</a></div>
