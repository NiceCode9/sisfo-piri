...
<div class="col-12 col-sm-6"><div class="form-floating"><select name="rombel_id" class="form-select" required><option value="">— Rombel —</option>@foreach($rombels as $r)<option value="{{ $r->id }}" @selected($exam->rombel_id==$r->id)>{{ $r->kelas->nama_kelas }} — {{ $r->tahunAjaran->nama_tahun_ajaran }}</option>@endforeach</select><label>Rombel</label></div></div>
<div class="col-12 col-sm-6"><div class="form-floating"><input type="text" name="name" value="{{ $exam->name }}" class="form-control" required /><label>Nama</label></div></div>
